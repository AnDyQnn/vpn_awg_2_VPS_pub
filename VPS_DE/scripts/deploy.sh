...
# След моста Xray (до 8.63): общий том с его состоянием. Контейнера моста нет
# с 8.63.0, читать эту папку некому.
if [ -d "$NODE_DIR/volumes/xray" ]; then
    rm -rf "$NODE_DIR/volumes/xray" && echo "[Deploy] Убран след моста Xray: volumes/xray"
fi
...
if command -v systemd-run >/dev/null 2>&1; then
    if systemd-run --on-active=15 --unit=vpn-updater-refresh --collect \
        ...
        systemctl restart "de-agent-updater" >/dev/null 2>&1; then
        ...
        echo "[Deploy]     Поправить руками: systemctl restart de-agent-updater"
    fi
fi
