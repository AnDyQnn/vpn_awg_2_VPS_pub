...
# ---------------- СЕТЬ ----------------
# --- ЖДЁМ, ПОКА УЗЕЛ ВСТАНЕТ НА НОГИ ---------------------------------------
#
# Проверка в первые секунды после пересоздания контейнера рисует катастрофу на
# ровном месте. Такой отчёт хуже, чем никакого: человек читает ошибки, решает,
# что всё легло, и лезет чинить работающее.
#
# Ждём до полутора минут, пока агент ответит. Не ответил — значит дело и правда
# плохо, и проверки ниже отработают честно.
SETTLE_LEFT=90
while [ "$SETTLE_LEFT" -gt 0 ]; do
    docker exec de_vpn_agent curl -sf --max-time 3 \
        http://127.0.0.1:8000/api/health >/dev/null 2>&1 && break
    sleep 3
    SETTLE_LEFT=$((SETTLE_LEFT - 3))
done
if [ "$SETTLE_LEFT" -lt 90 ] && [ "$SETTLE_LEFT" -gt 0 ]; then
    add_check CAT_DOCKER "Готовность узла к проверке" "ok" "поднялся за $((90 - SETTLE_LEFT))с"
fi
...
# Мгновенный замер ничего не значит: проверка часто идёт сразу после сборки.
# Смотрим среднее за минуту относительно числа ядер.
...
if [ -z "$SSHD_EFF" ]; then
    ...
    # Предупреждение про пароль было не про пароль, а про перебор — а его
    # закрывает fail2ban. Писать «уязвимо к брутфорсу» при живом страже значит
    # врать, и такие строки перестают читать вместе с настоящими.
    F2B_BANNED=""
...
' ' ')
            if systemctl is-active --quiet vpn-security-upgrade.timer 2>/dev/null; then
                ...
                add_check CAT_HOST "Заплатки безопасности ОС" "ok" "Ждут $SEC, ставятся сегодня ночью: $SEC_NAMES"
            else
                add_check CAT_HOST "Заплатки безопасности ОС" "warning" "Ждут $SEC, а ежедневный проход не заведён: $SEC_NAMES"
            fi
        else
