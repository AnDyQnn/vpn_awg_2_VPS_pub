...
async def connections_screen(update: Update, context: ContextTypes.DEFAULT_TYPE, uuid_val):
    ...
    kb = []
    if state["has_xray"]:
        # Подпись говорит, ЧТО уйдёт: владелец ведёт людей на обоих протоколах,
        # и «отправить доступ» не отвечает на вопрос, какой именно. А «ссылка»
        # тут вдобавок обманывала: по ней не переходят, её вставляют в
        # приложение.
        kb.append([InlineKeyboardButton("📨 Конфиг Xray",
...
# Маски, проверенные с узла родной проверкой `xray tls ping`: рукопожатие с SNI
# проходит, TLS 1.3, сертификат проверяется обычным доверенным хранилищем.
# Время отклика измерено оттуда же. Порядок — от меньшего риска к большему.
#
# Сбербанка здесь больше нет: он отдаёт сертификат российского удостоверяющего
# центра, которого нет в хранилищах телефонов, и клиент обрывает соединение.
# Проверять маску глазами в браузере бесполезно — в России такой сертификат
# доверенный, и владелец увидит «работает» там, где у людей не работает.
MASKS = [
