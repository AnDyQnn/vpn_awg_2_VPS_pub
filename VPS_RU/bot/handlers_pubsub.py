...
Имя узла, сертификат на него и на внутренние имена. Сертификат нужен странице
отказа (на свои имена она открывается без предупреждения браузера); без него
она работает на самоподписанном.

Настоящая работа (выпуск сертификата и таймер продления) делается на хосте:
scripts/public_sub.sh. Боту из контейнера ни systemd, ни letsencrypt
недоступны. Имя файлов осталось от прежней подписки наружу — оно зашито в
отчёты на уже работающих узлах, и менять его незачем.
"""
import json
import os
import time
...
def is_on():
    """Выпущен ли сертификат. Судим по файлу, а не по записи о намерении.
...
async def screen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Настройки связаны порядком: без имени нет хорошего сертификата, без
    сертификата нет покрытия внутренних имён.
    ...
    named = cert_named()
    if not on:
        lines.append("**Сертификат:** нет — страница отказа на самоподписанном")
    else:
    ...
    if not have:
        ...
        lines += ["", "**Дальше:** «Выпустить сертификат». Занимает несколько "
                      "минут."]
    elif not named:
    ...
    if on:
        ...
        kb.append([InlineKeyboardButton("🗑 Убрать сертификат",
                                        callback_data="psub_off")])
    else:
        kb.append([InlineKeyboardButton("🔑 Выпустить сертификат",
...
async def turn_on(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выпустить сертификат. Без подтверждений: здесь ничего не теряют."""
    query = update.callback_query
    await _ask_host("on")
    await db.log_event("Сертификат", "Владелец попросил выпустить сертификат")
    await query.answer("Выпускаю, это займёт несколько минут")
    await _wait_screen(update, context, "Выпускаю сертификат…")


async def turn_off(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Убрать сертификат. Спрашиваем: страница отказа вернётся на
    самоподписанный, и браузер снова начнёт предупреждать."""
    query = update.callback_query
    if context.user_data.get("psub_sure") != "off":
        context.user_data["psub_sure"] = "off"
        text = ("🗑 **Убрать сертификат?**\n\n"
                "Страница отказа вернётся на самоподписанный сертификат, и "
                "браузер снова начнёт предупреждать на наших страницах. "
                "Продление остановится.\n\n"
                "_Выпустить заново можно здесь же, в любой момент._")
        kb = [[InlineKeyboardButton("🗑 Да, убрать", callback_data="psub_off")],
    ...
    await _ask_host("off")
    await db.log_event("Сертификат", "Владелец убрал сертификат")
    await query.answer("Убираю")
    await _wait_screen(update, context, "Убираю сертификат и таймер продления…")
...
def status_line():
    ...
    if not is_on():
        return "🌐 *Домен и сертификаты:* сертификата нет"
    left = cert_days_left()
    tail = (", осталось %.1f сут." % left) if left is not None else ""
    if left is not None and left < 1:
        tail += " ⚠️"
    return "🌐 *Домен и сертификаты:* сертификат есть" + tail
...
def domain_ok(name):
    ...
    имя, принятое здесь, могло не совпасть с тем, что увидит сертификат или зона
    имён внутри туннеля.
    """
...
async def domain_screen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines += [
        ...
        "",
        "_Если имя ещё не отвечает, сертификат на него не выпустится: он "
...
async def domain_off(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await show_screen(
        query, context,
        ("🌐 Имя убрано — сертификат вернётся на адрес при следующем выпуске "
