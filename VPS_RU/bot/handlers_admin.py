...
async def _watch_de_update(app, pre_ts):
    """После команды обновления DE ждём ПОДТВЕРЖДЕНИЯ, что она реально обновилась: поллим
    /host/deploy_status до ~5 мин и ловим ts новее, чем был до команды (pre_ts). Агент во
    время деплоя пересоздаётся (краткая недоступность) — это нормально, ретраим. По завершении
    шлём админу ✅ с новым commit-hash, либо ⚠️ при таймауте (сборка/pull не удались, или на
    DE ещё старый агент без обратной связи — тогда подтверждение появится со следующего раза)."""
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{DE_AGENT_URL}/host/deploy_status", timeout=5) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                        if int(data.get("ts", 0) or 0) > pre_ts:
                            h = data.get("hash", "") or "?"
