...
# Своё имя узла. Отсюда его берут все, кому оно нужно, — подписка, ссылки на
# профили, маска входа и зона имён внутри туннеля.
#
# Разбор здесь, а не у каждого свой: имя приходит от человека, а человек
# вставляет ссылку целиком, с «https://», косой чертой и портом. Три разных
# разбора в трёх модулях означали бы, что где-то имя примут, а где-то нет, и
# концов не найти.
#
# Читается из окружения при каждом обращении, а не запоминается при импорте:
# имя задаётся из бота, доезжает в контейнер пересозданием, и модуль мог
# загрузиться до того.
_DOMAIN_RE = re.compile(
    r"^(?!-)[a-z0-9-]{1,63}(?<!-)(\.(?!-)[a-z0-9-]{1,63}(?<!-))+$")


def public_domain(raw=None) -> str:
    """Имя узла, пригодное к употреблению, или пустая строка.

    Пустая строка — не ошибка: без имени проект работает по адресу, и так
    живёт всякая копия, пока владелец не купил домен.
    """
    name = (os.getenv("PUBLIC_DOMAIN", "") if raw is None else raw).strip().lower()
    ...
    return name if _DOMAIN_RE.match(name) else ""
...
def copy_button(text, label="📋 Скопировать"):
    """Кнопка, которая кладёт текст в буфер по нажатию.

    Пометка кодом копирует текст только по долгому нажатию, и это знают не все.
    А ссылку вида `https://…` некоторые клиенты Telegram всё равно делают
    нажимаемой прямо в коде — человек тыкает, открывается браузер, и вместо
    подписки он видит гору base64. Так и вышло.

    Кнопка снимает обе беды разом: нажал — скопировалось, браузер не при чём.

    Поле `copy_text` появилось в Telegram позже, чем установленная у нас
    библиотека, поэтому передаём его напрямую. Библиотека кладёт такие поля в
    запрос как есть — проверено, получается ровно то, что ждёт Telegram.
    """
    from telegram import InlineKeyboardButton
    return InlineKeyboardButton(text=label,
                                api_kwargs={"copy_text": {"text": text}})
