...
async def profile(uuid_val=None):
    ...
    # Имена — в туннель, все, кроме перечисленных прямыми.
    #
    # Это второй экземпляр того же намерения: по сетям оно уже сказано
    # ноль-сетью в ProxyIp. Дублировать пришлось вот почему.
    #
    # В режиме TUN приложение читает из профиля прямые списки — это видно по
    # журналу владельца, `www.gosuslugi.ru` и адрес узла ушли мимо туннеля
    # ровно по нашим правилам, — а туннельные ПО СЕТЯМ не применяет ни одного,
    # даже узкую 10.13.13.0/24. Выходом по умолчанию при этом ставит прямой,
    # хотя `GlobalProxy: true` по документации означает обратное, и в примерах
    # самой документации этот флаг стоит вместе с непустыми туннельными
    # списками. То есть профиль у нас составлен правильно, а не применяется он
    # в одном конкретном режиме.
    #
    # Список по ИМЕНАМ мы не отправляли ни разу — он заполнялся только личными
    # маршрутами ключа, которых ни у кого нет. Поэтому неизвестно, разделяет ли
    # он судьбу сетевого. Если нет — этого достаточно: соединения приходят с
    # уже опознанным именем (видно в журнале: `tcp:web.telegram.org:443`), и
    # правило по имени решает всё, даже когда DNS не отвечает.
    #
    # `regexp:.*` — «любое имя»; порядок правил у приложения «block, direct,
    # proxy», поэтому исключения считаются раньше и остаются исключениями.
    sites = sorted(set(proxy_sites))
    prof["ProxySites"] = sites + ["regexp:.*"]
