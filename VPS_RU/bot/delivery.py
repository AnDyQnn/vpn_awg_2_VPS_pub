...
def to_self(chat_id, tg_id):
    """Выдаём тому же человеку, кто нажал кнопку?

    Владелец чаще всего выдаёт ключи себе, и его Telegram привязан ещё к
    полудюжине чужих ключей. Выдача устроена так: копия владельцу — всегда,
    потому что без привязанного Telegram ссылку надо передать как-то иначе, — и
    отдельно то же самое человеку. Совпади эти двое, и в один чат приходит два
    одинаковых QR и две одинаковые ссылки подряд.

    Сравниваем строками: из Telegram приходит число, из базы — то же число, но
    иногда строкой, и `==` между ними тихо даёт «разные».
    """
    return tg_id is not None and str(tg_id) == str(chat_id)
