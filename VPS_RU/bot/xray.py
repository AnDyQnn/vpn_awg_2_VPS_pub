...
# Чем сервер Xray разрешает имена, которые прислал клиент.
#
# Клиент часто отдаёт серверу не адрес, а имя: «соединись с www.youtube.com».
# Разрешать его серверу приходится самому, и без этой настройки он брал
# системный DNS контейнера. Тот ведёт через докер к провайдеру узла, то есть в
# российский DNS, а тот на заблокированное отвечает пустотой: на живом узле
# `www.youtube.com` получал NXDOMAIN. Проверено настоящим клиентом с живым
# ключом — российские сайты открывались, YouTube нет.
#
# У AmneziaWG этого не бывает: там имена разрешает наш резолвер на узле, и Xray
# теперь идёт к нему же. Он слушает всю сетевую область, в том числе петлю.
...
# лежит редко. Раньше это перекрывали запасные входы на чужих масках:
# приложение переходило на живой само. Запасные убраны по решению владельца,
# и теперь вход живёт ровно столько, сколько живёт заглушка — то есть бот:
# перезапуск бота на эти секунды закрывает и вход.
SELF_DEST = "self"
...
# Вход один — на 443, выглядит обычным HTTPS.
#
# Раньше их было три: основной и два запасных, на 2053 и 2083, с масками
# wildberries.ru и ozon.ru. Задумывалось так: заблокируют одну маску — приложение
# само перейдёт на живой вход. Убраны по решению владельца.
#
# Почему это правильно, а не потеря. Запасные держались на ЧУЖИХ сайтах: Reality
# берёт сертификат у сайта-маски на каждом рукопожатии, и вход живёт ровно
# столько, сколько маска отдаёт годный сертификат. Именно так у нас две недели
# не работал один из входов — и узнали мы об этом от людей. Основной вход с тех
# пор переехал на свою заглушку, где сертификат наш, и держать рядом два входа
# на чужой зависимости значило держать две двери, за которыми никто не следит.
# А два лишних открытых порта HTTPS на узле — это ещё и две лишние приметы.
#
# Если маску основного входа всё же понадобится сменить на чужую — выбирать её
# надо с оглядкой на то, что ниже.
#
# Сбербанк маской быть не может. Он отдаёт сертификат «Russian Trusted Root
# CA» — удостоверяющего центра, которого нет в доверенных хранилищах телефонов
# и браузеров. А в REALITY
...
# работает.
async def entries():
    """Входы Xray. Он один — основной, из настроек.

    Списком, а не одной парой, оставлено намеренно: по нему собираются и
    конфиг сервера, и подписка, и ссылки. Вернуть второй вход, если он
    когда-нибудь понадобится, — значит дописать сюда строку, а не переделывать
    всех, кто этим пользуется.

    Порт и маска берутся из настроек — владелец мог сменить их на экране.
    """
...
async def readiness():
    ...
    try:
        ...
        sub_out = False
    out.append((sub_out, False, "Публичная подписка",
                "пока закрыта, профиль обновляется только у подключённых к "
                "туннелю: изменения не доедут до того, у кого VPN как раз не "
                "работает", "psub_menu"))

    # Сертификат на внутренние имена. Без него работает всё, кроме одного: наши
    # собственные страницы открываются с предупреждением браузера — и страница
    # отказа фильтра, и «доступ закрыт», и выдача ключей.
    wild = False
    try:
        import handlers_pubsub
        wild = handlers_pubsub.wildcard_on()
    except Exception:
        pass
    out.append((wild, False, "Сертификат внутренних имён",
                "наши собственные страницы открываются с предупреждением "
                "браузера: на имена внутри туннеля нужен сертификат, "
                "покрывающий все поддомены", "psub_zone"))
    return out
...
async def build_config():
    ...
    for person in people:
        ...
        outbounds.append({
            ...
            "sendThrough": twin,
            # Имя, пришедшее от клиента, разрешает наш резолвер — см. SERVER_DNS.
    ...
    config = {
        ...
        "outbounds": [{"protocol": "freedom", "tag": "direct",
                       "settings": {"domainStrategy": "UseIPv4"}}] + outbounds,
        # Чем сервер разрешает имена, пришедшие от клиента. См. SERVER_DNS.
        "dns": {"servers": [SERVER_DNS], "queryStrategy": "UseIPv4"},
        "routing": {
            "domainStrategy": "AsIs",
...
async def profile_links(user_uuid):
    ...
    Сейчас вход один, и ссылка одна. Списком — чтобы не переделывать
    подписку, если вход когда-нибудь снова станет не один.
    """
    out = []
...
async def subscription_base() -> str:
    ...
    if domain:
        try:
            ...
            port = 2096
    if domain:
        # Свой сайт маской — значит подписка доезжает и по обычному 443.
        # Reality уводит туда всех, кто не прошёл проверку, а это ровно наши
        # клиенты подписки: они приходят обычным HTTPS, без ключа.
        #
        ...
        # а выглядело это как «VPN не работает».
        if (await db.get_setting("xray_dest")) == SELF_DEST:
            try:
                import decoy
                if decoy.is_up():
                    port = 443
