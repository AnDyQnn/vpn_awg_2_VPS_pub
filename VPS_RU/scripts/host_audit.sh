...
WG_CONTAINER=vpn_wireguard

# --- ЖДЁМ, ПОКА УЗЕЛ ВСТАНЕТ НА НОГИ ---------------------------------------
#
# Проверка, запущенная в первые секунды после пересоздания контейнера, рисует
# катастрофу на ровном месте: «ядро зависло», «wg0 не найден», «NAT
# отсутствует», «порт закрыт». Всё это неправда — узел просто поднимается.
#
# Такой отчёт хуже, чем никакого: человек читает десять ошибок, решает, что
# всё легло, и лезет чинить работающее. Проверено на живом: аудит через 33
# секунды после обновления дал 10 ошибок при целом туннеле и живых людях.
#
# Поэтому ждём до полутора минут, пока панель узла ответит. Не ответила —
# значит дело и правда плохо, и все проверки ниже отработают честно.
SETTLE_LEFT=90
while [ "$SETTLE_LEFT" -gt 0 ]; do
    docker exec "$WG_CONTAINER" curl -sf --max-time 3         http://127.0.0.1:8000/api/health >/dev/null 2>&1 && break
    sleep 3
...
if [ "${DOH_RULES:-0}" -ge 3 ]; then
    add_check CAT_VPN "Запрет обходного DNS" "ok" "${DOH_RULES} правил"
else
