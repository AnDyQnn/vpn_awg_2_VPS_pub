...
# Сертификат узла. Идёт при каждой выкладке и ничего не перевыпускает, пока
...
# поднялся бы с мёртвым сертификатом. Заодно снимаются следы прежней подписки
# наружу — правила охраны порта 2096, которых больше не нужно.
if [ -f "$PROJECT_ROOT/scripts/public_sub.sh" ]; then
    echo "[Deploy] Сертификат узла..."
    # Ошибку не глотаем молча: сертификат не обязан выпуститься (может не
    # быть свободного порта 80, может не ответить удостоверяющий центр), но
    # знать об этом надо.
    if ! bash "$PROJECT_ROOT/scripts/public_sub.sh" ensure "$NODE_DIR"; then
        echo "[Deploy] ⚠️  Сертификат не выпустился — см. строки выше."
        echo "[Deploy]     Страница отказа работает на самоподписанном, обновление в силе."
    fi
fi
...
#
# Идёт последним шагом: сверять имеет смысл то, что уже поднялось.
