...
# Сертификат узла (имя флага — от прежней подписки наружу). В файле одно
# слово: on или off. Скрипту нужны права хоста — он берёт сертификат и ставит
# таймер, и ничего из этого бот из контейнера сделать не может.
...
while true; do
    ...
    # 4.6 СЕРТИФИКАТ УЗЛА
    #     Выпустить или убрать сертификат и таймер продления. Это работа хоста:
    #     у бота нет ни systemd, ни доступа к letsencrypt. Флаг называется по
    #     прежней подписке наружу — его кладёт бот, менять имя незачем.
    if [ -f "$PUBSUB_FLAG" ]; then
        MODE=$(head -1 "$PUBSUB_FLAG" | tr -d '[:space:]')
        rm -f "$PUBSUB_FLAG"
        case "$MODE" in
            on|off)
                echo "[Updater] Сертификат узла: $MODE"
                ...
                ;;
            *)
                echo "[Updater] Сертификат узла: непонятная команда '$MODE'"
                ;;
        esac
    fi
