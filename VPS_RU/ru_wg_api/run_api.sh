...
(
  sleep "$STARTUP_QUIET"
  while true; do
    nice -n 15 bash /app/update_ru_ips.sh
    sleep 43200
  done
) &
...
#
# Под nice: при старте он читает списки категорий с диска и считает по ним
# отпечатки — под миллион имён на категорию. Ядро у узла одно, и эта работа
# отбирает его у того, что важнее: у панели и у настройки туннеля. Замерено —
# панель отвечала на 49-й секунде, и проверка после обновления не дожидалась.
#
# Приоритет ниже не значит «медленнее, когда никто не мешает»: пока ядро
# свободно, фильтр получает его целиком. Он уступает только тогда, когда за
