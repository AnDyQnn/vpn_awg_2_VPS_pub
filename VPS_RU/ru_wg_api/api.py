...
def setup_network():
    ...
    # 443 снаружи НЕ закрываем: там слушает подписка для клиентов на mihomo
    # (бот, clashsub.py). Без сертификата бот порт не открывает вовсе, так
    # что без него снаружи там просто пусто. Страница отказа при этом живёт
    # на 8443 и наружу не опубликована.
    ...
    # Внутренние сети Docker — не для пиров. За ними база (172.20.0.5:5432) и
    # сам хост через шлюз Docker; узел выпускал туда трафик туннеля, и любой
    # пир доставал до базы — держал только пароль. Проверено с пира. Бот и
    # узел ходят туда сами (OUTPUT), им это правило не мешает.
    _insert_before_accept(
