...
# Пределы считались под подписку — короткий текстовый ответ. Этот же порт
# раздаёт гео-файлы: двадцать семь мегабайт, которые телефон тянет в несколько
# потоков. Восьми соединений не хватало, человек получал обрыв, а приложение
# без гео-файлов считает профиль испорченным целиком — не работает НИЧЕГО.
# Плюс мобильный оператор держит за одним адресом тысячи абонентов: «один
# адрес» здесь не «один человек».
import re as _re
_m = _re.search(r"--connlimit-above (\d+)", text)
check("соединений хватает телефону с гео-файлами",
      bool(_m) and int(_m.group(1)) >= 32,
      "сейчас %s" % (_m.group(1) if _m else "не нашли"))
_r = _re.search(r"--hashlimit-above (\d+)/min", text)
check("частота не режет соседей по вышке",
      bool(_r) and int(_r.group(1)) >= 120,
      "сейчас %s/мин" % (_r.group(1) if _r else "не нашли"))
