...
# Это ломалось по-настоящему: пакеты человека к 10.13.13.1 попадали под
# замыкающий запрет его роли. Интернет по адресам работал, имена не
# разрешались: «подключается, но ничего не грузится».
...
# правило с «-i wg0»; здесь — второй замок, раньше разрешения узла целиком.
panel_idx = next((i for i, l in enumerate(body)
                  if "-d %s/32" % ns["NODE_IP"] in l and "--dport 8000" in l
                  and "REJECT" in l), None)
assert panel_idx is not None, "панель узла не закрыта в цепочке ролей"
...
# Привязки к интерфейсу тут нет: адрес назначения из туннельной сети —
# признак надёжнее.
assert "-i wg0" not in hook[0], "цепочка привязана к интерфейсу"

out_hooks = subprocess.run("iptables -S OUTPUT", shell=True,
                           capture_output=True, text=True).stdout
assert "WG_ACL" in out_hooks, \
    "цепочка ролей не подключена к трафику самого узла"
