...
# Привязки к интерфейсу тут быть НЕ должно: во время переезда пиры живут и на
# wg0, и на wg1. Адрес назначения из туннельной сети — признак надёжнее.
assert "-i wg0" not in hook[0], "привязка к интерфейсу не переживёт переезд"
