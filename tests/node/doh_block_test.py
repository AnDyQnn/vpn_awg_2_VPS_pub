...
# Люди на Xray ходят вторым путём: их трафик рождается на узле и через транзит
# не проходит вовсе. Без второй точки запрет их не коснулся бы.
assert hooks("FORWARD"), "запрет не подключён к транзиту"
assert hooks("OUTPUT"), (
    "запрет не подключён к трафику узла — людей на Xray он бы не коснулся")
