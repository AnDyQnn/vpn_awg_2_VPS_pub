...
async def main():
    ...
    print("\n=== вход один, запасных нет ===")
    # Запасные входы на чужих масках убраны по решению владельца. Упавшая
    # заглушка теперь закрывает вход целиком — это известная цена, а не
    # недосмотр, и тест держит её на виду.
    assert len(ways) == 1, ways
    print("  ", ways)
    print("вход один: ок")
    ...
    assert by_name["Публичная подписка"] is False
