...
async def main():
    ...
    check("объяснено, зачем она", "обновляться сами" in everything,
          "иначе человек добавит разовую ссылку и ничего не изменится")
    check("разовая ссылка тоже осталась", "vless://" in everything,
          "на первый раз и для приложений без подписок")
