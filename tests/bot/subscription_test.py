...
async def main():
    ...
    print()
    print("=== человеку уходит и конфиг, и адрес обновлений ===")
    # Раньше адрес подписки человеку не отдавался вовсе: он открывал его в
    # браузере и видел набор символов. Причина была в том, КАК он отдавался —
    # ссылкой. Отданный кодом, он ничего не открывает, а нажатие копирует.
    #
    # Отдавать его всё-таки нужно: сам по себе `vless://` не обновляется, и
    # человек навсегда остаётся на том, что ему выдали однажды. Список
    # исключений, смена маскировки, переезд узла — всё это приезжает подпиской.
    #
    # Порядок важен: сначала подключиться, потом добавить адрес. Подписка живёт
    # внутри туннеля, и прочитать её можно, только уже будучи подключённым.
    ...
    check("адрес обновлений тоже ушёл", url in everything,
          "без него человек остаётся на выданном однажды")
    check("он отдан кодом, а не ссылкой", "`" + url + "`" in everything,
          "ссылкой его открывали в браузере и видели набор символов")
    check("и он последним шагом, а не первым",
          everything.index("vless://") < everything.index(url),
          "подписка внутри туннеля: сперва подключиться, потом добавлять")
