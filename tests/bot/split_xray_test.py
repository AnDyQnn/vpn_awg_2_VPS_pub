...
async def main():
    ...
    hint = sent[-1]
    check("адрес обновлений попал в инструкцию", sub in hint,
          "без него человек навсегда остаётся на том, что выдали однажды")
    check("он отдан кодом", f"`{sub}`" in hint,
          "нажал — скопировалось")
    check("и сказано, зачем он", "мимо" in hint.lower() or "сами" in hint.lower())
    check("лишнего сообщения не появилось", len(sent) == 3,
          "их %d: картинка, ссылка, инструкция" % len(sent))
