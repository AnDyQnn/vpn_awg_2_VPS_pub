...
ALLOWED = {
    ...
    # Подводка к придуманному паролю: сразу за ней идёт сам пароль, который
    # копируется одним касанием, а за ним — сообщение с кнопками. Кнопки
    # посреди этой тройки только сбивают: жать надо после того, как пароль
    # вставлен в панель, а не до.
    ("handlers_pubsub.py", "Вот пароль"),
}
...
# startswith("client_plat_") ловит кнопки всех систем разом. Без этого
