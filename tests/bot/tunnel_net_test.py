...
async def main():
    ...
    # Общего «любое имя — в туннель» здесь быть не должно: отправляли
    # `regexp:.*`, приложение в TUN его не применило — как и туннельные списки
    # по сетям. Мёртвое правило в профиле, который читает человек, врёт ему о
    # том, что настроено.
    ps = prof.get("ProxySites") or []
    check("мёртвого правила по именам нет", "regexp:.*" not in ps,
          "проверено на живом: в TUN оно не применяется")
    check("исключения не попали в туннельные имена",
          not (set(prof["DirectSites"]) & set(ps)),
          "иначе одно и то же имя стоит в обоих списках")
