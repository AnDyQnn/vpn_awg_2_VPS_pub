...
async def main():
    ...
    # То же намерение по именам. В TUN приложение туннельные списки по сетям не
    # применяет вовсе — а соединения приходят с уже опознанным именем, и по
    # имени решение принять можно даже когда DNS молчит.
    ps = prof.get("ProxySites") or []
    check("любое имя — в туннель", "regexp:.*" in ps,
          "по сетям в TUN правила не применяются, по именам — не проверяли")
