...
def check_node_cert():
    ...
    неоткуда: страница отказа молча вернётся на самоподписанный.

    Отсюда видно только файл. Таймер продления живёт на хосте, его смотрит
    проверка хоста.
    """
    ...
    try:
        ...
        size = 0
    if not size:
        # Нет сертификата — страница отказа на самоподписанном. Не поломка.
        say("ok", "Сертификат узла", "не выпущен — страница отказа на самоподписанном")
        return
    ...
    if left is None:
        say("warning", "Сертификат узла", "срок неизвестен")
    elif left < 1:
        say("error", "Сертификат узла",
            "истекает меньше чем через сутки — продление не доехало")
    elif left < 3:
        say("warning", "Сертификат узла", "осталось %.1f сут." % left)
    else:
