...
# Присмотр за процессом Xray на мастере.
#
# У амнезии присмотр был с самого начала: пропало рукопожатие — узел лечится по
# ступеням. У Xray не было ничего: процесс поднимался при старте контейнера, и
# на этом всё. Упал через неделю — люди на нём без связи, пока не заметят.
#
# Лечим ОТДЕЛЬНО от узла и не даём влиять на решение о его лечении: перезапуск
# одного процесса не повод трогать всех остальных. Узел вернёт «работает», если
# всё в порядке, — вызов идемпотентный.
check_xray() {
    [ "$IS_MASTER" = "1" ] || return 0
    docker exec "$CONTAINER" curl -sf --max-time 8 -X POST \
        http://127.0.0.1:8000/api/xray/keep 2>/dev/null \
        | grep -q '"action":"поднят"' && log "Xray не работал — поднял"
    return 0
}
