...
# тишина. Поэтому открываем явно и здесь же, рядом с остальной настройкой
...
if command -v ufw >/dev/null 2>&1; then
    ...
    echo "[maintenance] Вход Xray разрешён в файрволе: 443."

    # Запасные входы 2053 и 2083 убраны по решению владельца. Правила на них
    # остались бы открытыми дверями, за которыми никто не слушает. Снимаем —
    # но только если порт действительно пуст: пока старый контейнер ещё
    # работает, он его держит, и правило снимется при следующем проходе.
    for XPORT in 2053 2083; do
        ...
        echo "[maintenance] Снято правило ufw на $XPORT/tcp: запасного входа больше нет."
    done
