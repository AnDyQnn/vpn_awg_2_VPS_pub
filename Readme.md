...
| Узел | Контейнер | Роль |
| :--- | :--- | :--- |
| Мастер | `vpn_wireguard` | AmneziaWG-сервер + FastAPI (управление пирами, маршрутизация) |
| Мастер | `vpn_bot` | Telegram-бот (живёт в сетевом пространстве WireGuard) |
| Мастер | `vpn_db` | PostgreSQL 15 (пользователи, статистика, логи, тикеты) |
| Клиент | `de_vpn_agent` | AmneziaWG-клиент + FastAPI Monitor (выход в интернет, метрики) |
