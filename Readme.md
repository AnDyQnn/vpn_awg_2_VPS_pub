...
- [<img src=".github/icons/spark.svg" width="18" height="18" align="top" alt=""> Xray — второй канал через панель 3X-UI](#xray-второй-канал)
...
iptables -t nat -A WG_ACL_WEB -s 10.13.13.2 -p tcp --dport 443 -j DNAT --to 10.13.13.1:8443
```
...
<a id="xray-второй-канал"></a>
## <img src=".github/icons/spark.svg" width="28" height="28" align="top" alt=""> Xray — второй канал через панель 3X-UI

**Необязателен и выключен по умолчанию.** Проект полностью работает на одном
AmneziaWG. Xray — второй способ подключиться рядом с ним: на случай, если у
чьего-то оператора AmneziaWG работает плохо.

Не свой Xray, а готовая панель [3X-UI](https://github.com/MHSanaei/3x-ui) в
соседнем контейнере: входы, подписки, статистика и шаблоны под блокировки в
ней уже есть и обновляются сообществом. Включается одной кнопкой в боте —
«Протоколы → Включить Xray», — дальше бот настраивает панель сам: меняет
заводской логин, заводит вход VLESS + Reality на 443 и подписку на имени узла,
кладёт в подписку сайты из списка обхода.

- **Сеть общая с узлом**: исходящее Xray идёт по его правилам — российское
  напрямую, остальное через Германию. Своего моста не нужно.
- **Два канала у одного человека**: карточка → «Каналы». Выдать Xray, прислать
  подписку, снять AmneziaWG после переезда, вернуть его обратно. Пауза и
  удаление действуют на оба канала.
- **Панель и её бот** — в тех же «Протоколах»: адрес, логин, токен бота панели
  и кнопка перехода в него.

Подробно — [`docs/xray.md`](docs/xray.md).
...
- выпустит **сертификат узла** Let's Encrypt (на IP-адрес, бесплатно и без
  домена) и поставит таймер продления — для страницы отказа.

После него в боте остаются только необязательные шаги — токен панелей, домен,
сертификат на внутренние имена. Бот сам показывает, чего не
