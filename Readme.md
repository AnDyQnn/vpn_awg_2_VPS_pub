...
По умолчанию мировой трафик Xray уходит тем же туннелем, что и трафик
AmneziaWG: Германия подключена обычным пиром, и два канала на деле — один.
Заблокируют амнезию по признаку протокола, и умрут оба.

Отдельной кнопкой включается свой путь на VLESS. Направление прежнее —
**подключается Германия**, и приходит на тот же порт, где сидят люди, ещё одним
посетителем среди остальных. Нет моста — люди автоматически идут прежним путём,
связь не прерывается.

На Германии мост живёт отдельным контейнером рядом с агентом: иначе перезапуск
агента ронял бы оба канала разом, и независимость осталась бы только на бумаге.

Одна честная оговорка: на этом пути **не работает лимит в пакетах в секунду** —
Xray считает байты, а пакеты не считает вовсе. Подробно, вместе с тем, как
устроен откат и учёт, — [`docs/xray.md`](docs/xray.md).
