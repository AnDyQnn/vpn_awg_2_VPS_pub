...
> обычного HTTPS, и на уровне DNS его не видно. Страница отказа показывается только для
> HTTP — подменить сертификат чужого сайта нельзя, не поставив свой корневой сертификат
> на каждое устройство, а это уже вскрытие чужого трафика; для HTTPS браузер покажет
> ошибку соединения. Фильтр рассчитан на обычную семейную историю, а не на того, кто
> целенаправленно обходит.
