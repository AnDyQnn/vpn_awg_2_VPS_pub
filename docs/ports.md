...
| `443/tcp` | бот: подписка Clash Mi | тот же ключ AmneziaWG ссылкой; открыт, только когда у узла есть сертификат ([clashsub.md](clashsub.md)) |
...
Свой Xray убран целиком: запасные 2053 и 2083, подписка наружу на 2096.
Правила `ufw` на эти порты снимает недельное обслуживание хоста, как только
убедится, что их никто не слушает (`scripts/ensure_host_maintenance.sh`). 443
теперь занят подпиской Clash Mi, его обслуживание не трогает.
...
таймаут. Сам 443 страница не занимает — проверяет
`tests/node/port_conflict_test.py`.
...
flowchart TB
    ...
    subgraph HOST["Хост VPS_RU"]
        ...
        subgraph NS["Одна сетевая область: vpn_wireguard + vpn_bot"]
            ...
            SUB["Подписка Clash Mi<br/>443 · https"]
        end
...
Им же подписывается подписка Clash Mi на 443. Без сертификата её нет вовсе:
в профиле закрытый ключ, открытым текстом его не отдаём.
