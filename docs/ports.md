...
2096 — общеизвестный запасной HTTPS и среди чужого трафика не выделяется.

Запасных входов Xray на 2053 и 2083 больше нет: они держались на чужих масках
(wildberries.ru, ozon.ru) и убраны по решению владельца. Вход один — 443, на
своей заглушке. Цена этого решения описана в `docs/xray.md`: упавшая заглушка
теперь закрывает вход целиком, перехода на запасной нет.
...
| — | мост Xray на Германии (`de_vpn_xray`) | портов не публикует: подключается сам, слушать ему нечего |
| `10085/tcp` | счётчики Xray | **только петле**; из них берётся учёт людей на своём канале в Германию |
...
flowchart TB
    ...
    subgraph HOST["Хост VPS_RU"]
        ...
        subgraph NS["Одна сетевая область: vpn_wireguard + vpn_bot"]
            ...
            XRAY["Xray<br/>443"]
            SUBOUT["Публичная подписка<br/>2096 · https"]
...
```mermaid
flowchart LR
    IN["Запрос на 2096"] --> R1{"Больше 64 соединений<br/>с одного адреса?"}
    R1 -->|да| DROP1["DROP"]
    R1 -->|нет| R2{"Больше 240 запросов<br/>в минуту с адреса?"}
    R2 -->|да| DROP2["DROP"]
...
## Публичная подписка: чем держится безопасность
