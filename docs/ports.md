...
| `443/tcp` | Xray в панели 3X-UI | вход; **открыт, только пока стек Xray включён** |
| `2096/tcp` | подписка 3X-UI | туда же; открыт вместе со стеком |
| `23232/tcp` | SSH | перенесён с 22, чтобы не собирать перебор паролей |

443 и 2096 опубликованы всегда, а пропускает их цепочка `XRAY_GATE` на узле:
стек выключен — в ней запрет, и снаружи порты молчат как закрытые. Подробно —
[`xray.md`](xray.md).

Прежний свой Xray убран в 8.63.0: запасные 2053 и 2083 больше не нужны.
Правила `ufw` на них снимает недельное обслуживание хоста, как только
убедится, что их никто не слушает (`scripts/ensure_host_maintenance.sh`).
...
| `2053/tcp` | панель 3X-UI | боту по петле; из туннеля — только ключам владельца (цепочка `XUI_PANEL`) |
...
таймаут. Сам 443 в сетевой области узла занят входом Xray из панели 3X-UI.
Страница, занявшая 443, не дала бы ему подняться — это стережёт `tests/node/port_conflict_test.py`.
